...
def main(load_path, image_path, laplace_path):
    ...
    plt.imshow(original_image, cmap = 'gray'),                              plt.xticks([]), plt.yticks([]), plt.figure()
    plt.imshow(laplace_image, cmap = 'gray'),                               plt.xticks([]), plt.yticks([]), plt.figure()
    plt.imshow(model_image.cpu().data.numpy().transpose((1,2,0)), cmap = 'gray'), plt.xticks([]), plt.yticks([]), plt.show()


if __name__ == '__main__':
    weights_path = 'models/model-10000.pt'
